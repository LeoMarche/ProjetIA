...
def save_crops(crops):
    ...
    for crop_idx in range(len(crops)):
        filename = "./output3/output-" + datetime_string
...
if __name__ == "__main__":
    ...
    def add_potential_crop(crop_tuple):
        potential_crops.append(recadrage3.crop_image(args.image, crop_tuple))


    for interest_cluster in interest_clusters:
        add_potential_crop(recadrage3.get_crop_tuple_one_center(ratio, saliency_map, initial_shape, interest_cluster))

    centroids = [i['centroid'] for i in interest_clusters]
    add_potential_crop(
        recadrage3.get_crop_tuple_least_square_distance_to_interest_points(ratio, saliency_map.shape, initial_shape,
                                                                           centroids))
    n_best_interest_points = 2
    if n_best_interest_points < len(centroids):
        add_potential_crop(
            recadrage3.get_crop_tuple_least_square_distance_to_best_interest_points(ratio, saliency_map.shape,
                                                                                    initial_shape, interest_clusters,
                                                                                    n_best_interest_points))
    add_potential_crop(recadrage3.get_crop_tuple_sigmoid_based(ratio, saliency_map, initial_shape))
    #add_potential_crop(recadrage3.get_crop_tuple_contour_based_random(args.image, initial_shape))

    # Utilisation de la nouvelle logique de recadrage basée sur la fonction sigmoïde
    add_potential_crop(recadrage3.get_crop_tuple_sigmoid_based(ratio, saliency_map, initial_shape))

    add_potential_crop(recadrage3.get_crop_tuple_random(ratio, saliency_map, initial_shape))
