...
def get_crop_tuple_using_contours(image, initial_shape):
    ...
    if not contours:
        return (0, 0, initial_shape[1], initial_shape[0])  # Si aucun contour n'est trouvé, renvoyer l'image entière
...
def get_crop_tuple_contour_based_random(image, initial_shape):
    img = cv2.imread(image, cv2.IMREAD_GRAYSCALE)
    _, thresh = cv2.threshold(img, 128, 255, cv2.THRESH_BINARY)
    ...
    # Choisir un contour aléatoire parmi les contours détectés
    random_contour = random.choice(contours)

    # Calculer le rectangle englobant le contour choisi
